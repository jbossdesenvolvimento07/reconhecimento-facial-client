...
import { Accordion, UncontrolledAccordion, AccordionItem, AccordionHeader } from 'reactstrap'
            ...
            elements.push(
                ...
                <div className="card mb-3" style={{minWidth: '100%'}} key={associado.id}>
                    <div className="card-body">
                        ...
                        {associado.statusRF === 'A' ? <small style={{color: '#198754'}}><i className="bi bi-check2-square"></i>&nbsp;RECONHECIMENTO FACIAL</small> : <small style={{color: '#dc3545'}}><i className="bi bi-check2-square"></i>&nbsp;RECONHECIMENTO FACIAL</small>}
                    </div>
    ...
    return (
        <div className='container-fluid d-flex flex-column align-items-center' style={{ marginTop: '70px' }}>
            ...
            <div className='titulo w-100 mb-4'>
                <h2 className='mb-0 fw-bolder'>PESQUISA</h2>
                <p className='text-muted border-bottom mb-0'>Pesquise os dados cadastrais de associados.</p>
            </div>

            <h4>PESQUISA POR:</h4>
            <div className="btn-group" role="group">

                <input type="radio" className="btn-check" name="btnradio" id="btnradio1" autoComplete="off" onClick={() => { setTipoFiltro('nome') }} defaultChecked />
                <label className="btn btn-outline-secondary" htmlFor="btnradio1">Nome</label>
                <input type="radio" className="btn-check" name="btnradio" id="btnradio2" autoComplete="off" onClick={() => { setTipoFiltro('cpf') }} />
                <label className="btn btn-outline-secondary" htmlFor="btnradio2">CPF</label>
                <input type="radio" className="btn-check" name="btnradio" id="btnradio3" autoComplete="off" onClick={() => { setTipoFiltro('cod') }} />
                <label className="btn btn-outline-secondary" htmlFor="btnradio3">Código</label>
                <input type="radio" className="btn-check" name="btnradio" id="btnradio3" autoComplete="off" onClick={() => { setTipoFiltro('cota') }} />
                <label className="btn btn-outline-secondary" htmlFor="btnradio3">Cota</label>
